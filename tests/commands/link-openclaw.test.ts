...
import { OPENCLAW_DIR, OPENCLAW_BACKUP_DIR } from '@/_defs';
...
const { linkOpenclaw, unlinkOpenclaw } = await import('@/link/openclaw');
...
describe('linkOpenclaw', () => {
    ...
    it('skills 디렉토리 존재 시 심볼릭 링크 생성', async () => {
        ...
        expect(vol.lstatSync(path.join(OPENCLAW_DIR, 'skills')).isSymbolicLink()).toBe(true);
    });
